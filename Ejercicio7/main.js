...
function calcPromedio() {
  if (notas.length > 4) {
    document.getElementById("estado").textContent =
      "no puedes calcular el promedio pues solo 4 notas se permiten.";
  }
  else if (notas.length == 4) {
    let promedio;
    let sumatoria = notas.reduce(function (acumulador, siguienteValor) {
      //me suma los elementos del array de int
    ...
    promedio >= 61
      ? (document.getElementById(
          "estado"
        ).textContent = `Haz Aprobado con ${promedio}`)
      : (document.getElementById(
          "estado"
        ).textContent = `Haz Reprobado con ${promedio}`);
...
document.getElementById("evaluar").addEventListener("submit", function (event) {
  ...
  document.getElementById(
    "mensaje"
  ).textContent = `Cantidad de Notas ingresadas: ${contador}`;
